  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        setLastLoginDate(parsedUser.data.loginDate);
        setLastLoginTime(parsedUser.data.loginTime);
        setEmployeeId(parsedUser.data._id);
        setName(parsedUser.data); // Assuming the parsed user contains name data
      } catch (error) {
  ...
  const fetchAttendance = async () => {
    try {
      const response = await axios.post(
        `http://localhost:8000/api/v1/user/${employeeId}/attendence/details`
      );
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-xl text-red-600 font-bold">{totalAbsentDays}</p>
          </div>
        </div>
      </div>
          ...
          <table className="min-w-full table-auto">
            ...
            <tbody className="text-gray-700">
              ...
              <tr className="hover:bg-gray-100">
                ...
                <td className="px-6 py-4 text-center">{totalPresentDays}</td>
                <td className="px-6 py-4 text-center">{totalAbsentDays}</td>
